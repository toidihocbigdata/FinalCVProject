...
def get_vectors(image, points):
    ...
    # with np.load('webcam_calibration_ouput_new.npz') as X:
    #     mtx, dist, _, _ = [X[i] for i in ('mtx','dist','rvecs','tvecs')]
    with open("camera_parameters.json", 'r') as f:
        mtx = np.array(json.load(f))
    with open("distortion.json","r") as g:
        dist = np.array(json.load(g))
    ...
    imgp_new = np.zeros((4,1,2), dtype="float32")
    ...
    # rvecs, tvecs, _ = cv2.solvePnPRansac(objp, imgp_new, mtx, dist)
    corner2 = cv2.cornerSubPix(gray,imgp,(11,11),(-1,-1),criteria)
    _, rvecs, tvecs, _ = cv2.solvePnPRansac(objp, corner2, mtx, dist)
